...
import com.hegunhee.maplemfinder.core.data.BuildConfig
import com.hegunhee.maplemfinder.core.data.api.MapleMApi
import com.hegunhee.maplemfinder.core.data.util.MapleMMoshiName
...
class ApiModule {
    ...
    @Singleton
    @Provides
    @Named(MapleMMoshiName)
    fun provideMapleMMoshi() : Moshi = Moshi.Builder().add(KotlinJsonAdapterFactory()).build()

    @Singleton
    @Provides
    fun provideMapleMApi(
        @Named(MapleMMoshiName) moshi : Moshi
    ) : MapleMApi {
